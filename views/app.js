...
function renderComunicados(comunicados, erros, completosSet) {
  ...
  comunicados
    .filter(
      (com) =>
        com.erros.includes("Cartório de origem ausente") && com.nome_registrado,
    )
    .forEach((com) => {
      const nomeCard = document.createElement("div");
      nomeCard.className = "conjuge-card";
    
      const termo = com.parte2.match(/termo\s+(\d+)/i)?.[1] || "?";

      // captura a letra A ou B (tipo de livro)
      const matchLivro = com.parte2.match(/livro\s+([AB])/i);
      const tipo = matchLivro?.[1] === 'A' ? "Nascimento" :
                   matchLivro?.[1] === 'B' ? "Casamento" : "?";
      
      // captura o número do livro (ex: livro B nº 13 → 13)
      const livro = com.parte2.match(/livro\s+[AB]\s*(?:n[oº]+)?\s*([\w-]+)/i)?.[1] || "?";
      
      nomeCard.innerHTML = `
        <div class="card-header">
          <strong class="nome">${com.nome_registrado}</strong>
          <div class="origem-info">
            <span>${tipo}</span> • <span>Livro ${livro}</span> • <span>Termo ${termo}</span>
          </div>
        </div>
        ...
        <div style="display:flex; gap:6px; flex-wrap: wrap;">
          ...
          <button class="btn-copiar"
                  ...
                  title="${com.parte2.replace(/"/g, '&quot;')}">
            🔍 Ir para
          </button>
        </div>
        
        <button onclick="reorganizarComunicados()" style="margin: 10px 0; padding: 6px 12px;">
  🔄 Reorganizar comunicados
</button>
...
escreventeSelect.addEventListener(
  "change",
  (e) => (escrevente = e.target.value),
);
...
document.getElementById("exportarPDF").addEventListener("click", async () => {
  const cards = [...document.getElementById("resultados").children].filter(
...
function reorganizarComunicados() {
  if (!currentSessionId) return alert("Nenhuma sessão carregada.");

  const sessao = JSON.parse(localStorage.getItem("session_" + currentSessionId));
  if (!sessao) return alert("Sessão não encontrada.");

  sessao.comunicados.sort((a, b) => {
    const livroA = a.parte2.match(/livro\s+([AB])/i)?.[1] ?? '';
    const livroB = b.parte2.match(/livro\s+([AB])/i)?.[1] ?? '';
    const tipoA = livroA === 'A' ? 0 : livroA === 'B' ? 1 : 99;
    const tipoB = livroB === 'A' ? 0 : livroB === 'B' ? 1 : 99;

    if (tipoA !== tipoB) return tipoA - tipoB;

    const cartA = parseInt(a.cartorio_origem?.match(/\d+/)?.[0]) || 99;
    const cartB = parseInt(b.cartorio_origem?.match(/\d+/)?.[0]) || 99;

    return cartA - cartB;
  });

  renderComunicados(sessao.comunicados, sessao.erros, new Set(sessao.completos));
}
